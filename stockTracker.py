...
API_KEY = '5ed0e13cf4f347f9a90f159c9eb8ef37'
...
class NewsPanel(wx.Panel):
  ...
  def __init__(self, parent):
    wx.Panel.__init__(self, parent)
    self.SetBackgroundColour("gray")
    self.sources_list = wx.ListCtrl(self, 
    style=wx.LC_REPORT | wx.BORDER_SUNKEN)
    self.sources_list.InsertColumn(0, "Source", width=200)
    self.news_list = wx.ListCtrl(self, 
    size = (-1 , - 1),
    style=wx.LC_REPORT | wx.BORDER_SUNKEN)
  ...
  def getNews(self, source):
    with urllib.request.urlopen("httpn://newsapi.org/v2/top-headlines?sources="+ source + "&apiKey=" + API_KEY) as response:
  ...
  def getNewsSources(self):
    with urllib.request.urlopen("http://newsapi.org/v2/sources?language=en&apiKey=" + API_KEY) as response:
      response_text = response.read()   
      encoding = response.info().get_content_charset('utf-8')
      JSON_object = json.loads(response_text.decode(encoding))            
      #return JSON_object
